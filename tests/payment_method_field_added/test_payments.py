class TestPayments:
    async def test_create_payment_with_payment_method_id(self, auth_client, user_id, cell_id, payment_method_id):
    ...
    async def test_create_payment_without_payment_method_id(self, auth_client, user_id, cell_id):
        rental = await auth_client.post("/api/v1/rentals/", json={
            "user_id": user_id, "cell_id": cell_id, "price_per_hour": 60.00,
        })
        rental_id = rental.json()["id"]
    ...
    async def test_amount_is_float(self, auth_client, user_id, cell_id):
        rental = await auth_client.post("/api/v1/rentals/", json={
            "user_id": user_id, "cell_id": cell_id, "price_per_hour": 60.00,
        })
        rental_id = rental.json()["id"]
    ...
    async def test_filter_by_rental_id(self, auth_client, user_id, cell_id):
        rental = await auth_client.post("/api/v1/rentals/", json={
            "user_id": user_id, "cell_id": cell_id, "price_per_hour": 60.00,
        })
        rental_id = rental.json()["id"]
        ...
        resp = await auth_client.get(f"/api/v1/payments/?rental_id={rental_id}")
        data = resp.json()
        assert len(data) >= 1
        for p in data:
    ...
    async def test_filter_by_status(self, auth_client, user_id, cell_id):
        ...
        resp = await auth_client.get("/api/v1/payments/?status=PENDING")
        data = resp.json()
        assert len(data) >= 1
        for p in data:
            assert p["status"] == "PENDING"

    async def test_payment_not_found(self, auth_client):
        resp = await auth_client.get("/api/v1/payments/00000000-0000-0000-0000-000000000000")
