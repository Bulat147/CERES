class TestLockerCells:
    async def test_hourly_price_is_float(self, client, station_id):
        resp = await client.post("/api/v1/locker-cells/", json={
    ...
    async def test_title_field(self, client, station_id):
    ...
    async def test_title_default_empty(self, client, station_id):
        resp = await client.post("/api/v1/locker-cells/", json={
    ...
    async def test_filter_by_station_id(self, client):
        ...
        resp = await client.get(f"/api/v1/locker-cells/?station_id={s1_id}")
        data = resp.json()
        assert len(data) == 2
        for cell in data:
            assert cell["station_id"] == s1_id
        resp = await client.get(f"/api/v1/locker-cells/?station_id={s2_id}")
        assert len(resp.json()) == 1

    async def test_status_field_values(self, client, station_id):
        resp = await client.post("/api/v1/locker-cells/", json={
    ...
    async def test_station_id_present_in_response(self, client, station_id):
        resp = await client.post("/api/v1/locker-cells/", json={
    ...
    async def test_create_cell_no_station_404(self, client):
        resp = await client.post("/api/v1/locker-cells/", json={
    ...
    async def test_get_cell_not_found(self, client):
        resp = await client.get("/api/v1/locker-cells/00000000-0000-0000-0000-000000000000")
        assert resp.status_code == 404
